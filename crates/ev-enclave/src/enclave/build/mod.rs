pub mod error;
use crate::enclave::{
    common::{resolve_output_path, OutputPath},
    config::ValidatedEnclaveBuildConfig,
    docker::error::DockerError,
    docker::parse::{Directive, DockerfileDecoder, EnvVar, Mode},
    docker::utils::{
        create_combined_docker_entrypoint, verify_docker_is_running, write_command_to_script,
    },
    nitro,
};
...
async fn process_dockerfile<R: AsyncRead + std::marker::Unpin>(
    ...
    let user_service_builder = create_combined_docker_entrypoint(last_entrypoint, last_cmd)
        .map(|entrypoint| build_user_service(entrypoint, wait_for_env, last_user, user_env_vars))?;
    ...
    let injected_directives = vec![
        ...
        Directive::new_run(write_command_to_script(
    ...
    Ok([
        instructions,
        injected_directives,
        vec![Directive::new_run(write_command_to_script(
...
pub fn build_user_service(
    ...
    let user_service_runit_wrapper = write_command_to_script(
...
mod test {
    ...
    use crate::enclave::{
        cert::CertValidityPeriod, config::EgressSettings, config::ScalingSettings,
        config::ValidatedEnclaveBuildConfig, config::ValidatedSigningInfo, docker, nitro,
        test_utils,
    };
    use std::iter::zip;
