use crate::enclave::build::build_enclave_image_file;
use crate::enclave::common::{prepare_build_args, CliError};
use crate::enclave::config::{read_and_validate_config, BuildTimeConfig};
use crate::enclave::docker::command::get_source_date_epoch;
use crate::enclave::version::check_version;
use crate::enclave::version::get_runtime_and_installer_version;
use clap::Parser;
...
pub async fn run(build_args: BuildArgs) -> exitcode::ExitCode {
    ...
    crate::enclave::common::save_enclave_config(&enclave_config, &build_args.config);

    if enclave_config.debug {
        crate::enclave::common::log_debug_mode_attestation_warning();
    }
