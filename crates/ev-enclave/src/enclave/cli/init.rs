use crate::enclave::{
    api,
    api::enclave::{CreateEnclaveRequest, Enclave, EnclaveApi},
    api::AuthMode,
    cert,
    common::CliError,
    config::{default_dockerfile, EgressSettings, EnclaveConfig, ScalingSettings, SigningInfo},
...
async fn init_local_config(init_args: InitArgs, created_enclave: Enclave) -> exitcode::ExitCode {
    ...
    if initial_config.signing.is_none() {
        ...
        match cert::create_new_cert(output_path, cert::DistinguishedName::default()) {
...
mod init_tests {
    use super::*;
    use crate::enclave::api::enclave::EnclaveState;
