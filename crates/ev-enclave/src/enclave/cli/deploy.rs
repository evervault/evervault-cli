use crate::enclave::{
    api::{self, assets::AssetsClient, client::ApiErrorKind, enclave::EnclaveApi, AuthMode},
    build::build_enclave_image_file,
    common::{self, prepare_build_args, CliError, OutputPath},
...
pub async fn run(deploy_args: DeployArgs) -> exitcode::ExitCode {
    ...
    if enclave_config.debug {
        common::log_debug_mode_attestation_warning();
    }
    ...
    common::save_enclave_config(&enclave_config, &deploy_args.config);
