...
fn encrypt(client: &CryptoClient, value: &mut Value) {
  if value.is_object() {
    value.as_object_mut().unwrap().values_mut().for_each(|val| encrypt(client, val));
  } else if value.is_array() {
    value.as_array_mut().unwrap().iter_mut().for_each(|val| encrypt(client, val));
  } else {
    let mut val = value.clone();
    let to_encrypt = convert_value_to_string(&value);
    let encrypted_data_result = client.encrypt(
      to_encrypt, 
...
fn decrypt(client: &CryptoClient, value: &mut Value) {
  if value.is_object() {
    value.as_object_mut().unwrap().values_mut().for_each(|val| decrypt(client, val));
  } else if value.is_array() {
    value.as_array_mut().unwrap().iter_mut().for_each(|val| decrypt(client, val));
...
async fn encryption_handler(mut request: Request<CryptoClient>) -> tide::Result {
  ...
  if let Ok(mut payload) = request.body_json().await {
    ...
    let res = tide::Response::builder(400)
      .content_type(mime::JSON)
      .body(serde_json::json!({ "message": "Failed to parse encrypt request payload as JSON" }))
      .build();
    Ok(res)
  }
}
...
async fn decryption_handler(mut request: Request<CryptoClient>) -> tide::Result {
  ...
  if let Ok(mut payload) = request.body_json().await {
    ...
    let res = tide::Response::builder(400)
      .content_type(mime::JSON)
      .body(serde_json::json!({ "message": "Failed to parse decrypt request payload as JSON" }))
      .build();
    Ok(res)
  }
}
