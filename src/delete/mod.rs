...
use crate::progress::{get_tracker, ProgressLogger, StatusReport, poll_fn_and_report_status};
mod error;
...
async fn watch_deletion(cage_api: CagesClient, cage_uuid: &str, progress_bar: impl ProgressLogger) {
  async fn check_delete_status(cage_api: CagesClient, args: Vec<String>) -> Result<StatusReport, DeleteError> {
  ...
  let _ = poll_fn_and_report_status(cage_api, check_delete_args, check_delete_status, progress_bar).await;
}
