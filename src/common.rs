...
impl BuildArg {
    ...
    // defined as an associated function for convenient chaining
    pub fn to_command_line_arg(arg: &Self) -> std::ffi::OsString {
        format!("--build-arg {}", arg.to_string()).into()
    }
}

impl std::fmt::Display for BuildArg {
...
impl clap::builder::ValueParserFactory for BuildArg {
    type Parser = BuildArgParser;
...
impl clap::builder::TypedValueParser for BuildArgParser {
    type Value = BuildArg;
    ...
    fn parse_ref(
        ...
        let given_string = value.to_string_lossy();
        let mut tokens = given_string.split("=").into_iter();
        ...
        let key = match tokens.next() {
            ...
            None => {
                return Err(clap::Error::raw(
                    ...
                    "No key found for build arg",
                ));
            }
        };

        let value = tokens.next().map(String::from);
        Ok(BuildArg { key, value })
    }
}
